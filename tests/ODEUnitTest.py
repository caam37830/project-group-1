...
class TestODEs(unittest.TestCase):
    ...
    def test_sums(self):
        ...
        self.assertTrue(np.abs(np.maximum(diff)) < .005 ) #We test to see if this is close enough to zero
    ...
    def test_derivative_sums(self): 
        ...
        self.assertTrue(np.abs(np.maximum(sum_array)) < .005 ) #We test to see if this is close enough to zero
